...
import CardProd from '../MarqueProducts/ProdCard';
import FormatListBulletedIcon from '@mui/icons-material/FormatListBulleted';
import GridViewIcon from '@mui/icons-material/GridView';
import ProdCard2 from '../MarqueProducts/ProdCard2';
function CategorieProducts({produitAchete,setproduitAchete}) {
  let [pageContent,setpageContent]=useState([])
  let {SubCategorieName,CategorieName}=useParams()
  console.log(SubCategorieName,CategorieName)
  let [CategorieProd,setCategorieProd]=useState([])
...
let [choosed,setChoosed]=useState(1)
 useEffect(()=>{
    ...
    const getProduits=async()=>{
      try {
        const res= await axios.get(`/api/Products/categorieProducts//${CategorieName}`)
        // const resCat=await axios.get(`/api/Products/${MarqueName}/Categories`)
        // setMarquCategories(resCat.data)
         setCategorieProd(res.data)
  ...
  let changeContent=(e,n)=>{
    e.preventDefault()
    if (n==1)
    setpageContent(CategorieProd.slice(0,2))
    else 
    setpageContent(CategorieProd.slice(n,n+2))
    setChoosed(n)
  }
// il faut mettre à jr le contenue setpageContent(CategorieProd.slice(0,2)) et mettre a jr la fct des buttons
//   tri par categorie
...
let res=[]
  switch (choix) {
    // NomProduit
      ...
      break;
      case "PrixCroissant":
         res= await axios.get(`/api/Products/CrPrix/`)
         console.log(res)
        setCategorieProd(res.data)

      break;
      case "PrixDecroissant":
         res= await axios.get(`/api/Products/DecPrix`)
        setCategorieProd(res.data)

      break;
      case "NomProduit":
        res= await axios.get(`/api/Products/NomProd`)
            setCategorieProd(res.data)
        break
...
// get nb botton
      nbButt=CategorieProd.length%2==0?CategorieProd.length/2:( Math.floor(CategorieProd.length/2)+1)
...
// +( choosed && "active")
  return (
    <div>
       {CategorieProd?<div className="Container_fx_Col w_100 Items_C JC_C">
        <span className='T_Bold M_1 '>Tous les produits de la Categorie: <span className='txt_red'> {} </span></span> 
            ...
            {x?<div className="Container_fx Flex_W">
              {pageContent.map(el=><ProdCard2 produit={el} produitAchete={produitAchete}  setproduitAchete={setproduitAchete}/>)}</div>:
